...
public class zkrollup32 extends CircuitGenerator {
  ...
  public zkrollup32() {
    super("zkrollup32");
    __generateCircuit();
    this.__evaluateSampleRun(new SampleRun("Sample_Run20", true) {
      public void pre() {
        ...
        new_data[13].mapValue(new BigInteger("0698244670940040474041342456690461912857457038217542443972543453394033105057"), CircuitGenerator.__getActiveCircuitGenerator().__getCircuitEvaluator());
        new_data[14].mapValue(new BigInteger("015738495716095711370531636491932792314965601597180587243645106382818753554"), CircuitGenerator.__getActiveCircuitGenerator().__getCircuitEvaluator());
        new_data[15].mapValue(new BigInteger("0705123809409757360278735222993763455804567653542562939383529169868273762079"), CircuitGenerator.__getActiveCircuitGenerator().__getCircuitEvaluator());
        new_data[16].mapValue(new BigInteger("463130995984243733666683573867942426343778730637472889833268551129368259027"), CircuitGenerator.__getActiveCircuitGenerator().__getCircuitEvaluator());
        new_data[17].mapValue(new BigInteger("0384618048489970178443210252570737795068270741721864290073710660098905205933"), CircuitGenerator.__getActiveCircuitGenerator().__getCircuitEvaluator());
        new_data[18].mapValue(new BigInteger("0237858706675528743001139037690797540756047048303280819655294354971824608510944"), CircuitGenerator.__getActiveCircuitGenerator().__getCircuitEvaluator());
        new_data[19].mapValue(new BigInteger("735469231532263072208205615042729418962016635954007166143292137895250684489925"), CircuitGenerator.__getActiveCircuitGenerator().__getCircuitEvaluator());

        for (int i = 20; i < 32; i++) {
          new_data[i].mapValue(new BigInteger("0"), CircuitGenerator.__getActiveCircuitGenerator().__getCircuitEvaluator());
        }
        // ECDSA输入 
  ...
  public void __init() {
    ...
    leaf = (UnsignedInteger[]) UnsignedInteger.createZeroArray(CircuitGenerator.__getActiveCircuitGenerator(), new int[]{32}, 256);
    leafNode = new Leaf[32];
    new_data = (UnsignedInteger[]) UnsignedInteger.createZeroArray(CircuitGenerator.__getActiveCircuitGenerator(), new int[]{32}, 256);
  ...
  public void outsource() {
    ...
    int j = 0;
    for (int i = 0; i < 32; i++) {
      branch2[i] = new MerkleTree();
      branch2[i].add(leafNode[j], new Leaf(new_data[j].copy(256)));
      j = j + 1;
    }
    ...
    for (int i = 0; i < 16; i++) {
      ...
      branch3[i].add(branch2[j], branch2[j + 1]);
      j = j + 2;
    }
  ...
  public UnsignedInteger[] pointMultiply(UnsignedInteger[] p1, UnsignedInteger n, UnsignedInteger a, UnsignedInteger mult) {
    ...
    for (int i = binMult.length - 1; i >= 0; i--) {
      {
        Bit bit_a0o0wb = bittes[i].copy();
        boolean c_a0o0wb = CircuitGenerator.__getActiveCircuitGenerator().__checkConstantState(bit_a0o0wb);
        if (c_a0o0wb) {
          if (bit_a0o0wb.getConstantValue()) {
            if (flag == false) {
              flag = true;
              binMultLen = i;
            }
          ...
          ConditionalScopeTracker.push(bit_a0o0wb);
          if (flag == false) {
            flag = true;
            binMultLen = i;
          }
    ...
    for (int c = binMultLen; c >= 0; c--) {
      {
        Bit bit_a0q0wb = binMult[c].isEqualTo(num[0]).copy();
        boolean c_a0q0wb = CircuitGenerator.__getActiveCircuitGenerator().__checkConstantState(bit_a0q0wb);
        if (c_a0q0wb) {
          if (bit_a0q0wb.getConstantValue()) {
            if (set) {
          ...
          ConditionalScopeTracker.pushMain();
          ConditionalScopeTracker.push(bit_a0q0wb);
          if (set) {
            val = pointAddition(val, doubledP, n.copy(64));
          } else {
            val = doubledP;
  ...
  public UnsignedInteger modInv(UnsignedInteger b, UnsignedInteger q, UnsignedInteger p) {
    ...
    for (int i = qq.length - 1; i >= 0; i--) {
      {
        Bit bit_a0h0fc = qq[i].copy();
        boolean c_a0h0fc = CircuitGenerator.__getActiveCircuitGenerator().__checkConstantState(bit_a0h0fc);
        if (c_a0h0fc) {
          if (bit_a0h0fc.getConstantValue()) {
            if (flag == false) {
              flag = true;
              fl = i;
            }
          ...
          ConditionalScopeTracker.pushMain();
          ConditionalScopeTracker.push(bit_a0h0fc);
          if (flag == false) {
            flag = true;
            fl = i;
          }
    ...
    for (int i = 0; i <= fl; i++) {
      {
        Bit bit_a0j0fc = qq[i].copy();
        boolean c_a0j0fc = CircuitGenerator.__getActiveCircuitGenerator().__checkConstantState(bit_a0j0fc);
        if (c_a0j0fc) {
          if (bit_a0j0fc.getConstantValue()) {
          ...
          ConditionalScopeTracker.pushMain();
          ConditionalScopeTracker.push(bit_a0j0fc);
          ans.assign(ans.mul(b).mod(p), 64);
