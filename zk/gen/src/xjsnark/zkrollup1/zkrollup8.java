...
public class zkrollup8 extends CircuitGenerator {
  ...
  public zkrollup8() {
    super("zkrollup8");
    __generateCircuit();
    this.__evaluateSampleRun(new SampleRun("Sample_Run7", true) {
      public void pre() {
        ...
        new_data[5].mapValue(new BigInteger("02743051463395398047635648845769997499355142011188778433723235283732013954197"), CircuitGenerator.__getActiveCircuitGenerator().__getCircuitEvaluator());
        new_data[6].mapValue(new BigInteger("4785074797473186494580334878359280208065580933170183206872558102038099558780"), CircuitGenerator.__getActiveCircuitGenerator().__getCircuitEvaluator());
        new_data[7].mapValue(new BigInteger("0"), CircuitGenerator.__getActiveCircuitGenerator().__getCircuitEvaluator());
  ...
  public void __init() {
    ...
    leaf = (UnsignedInteger[]) UnsignedInteger.createZeroArray(CircuitGenerator.__getActiveCircuitGenerator(), new int[]{8}, 256);
    leafNode = new Leaf[8];
    new_data = (UnsignedInteger[]) UnsignedInteger.createZeroArray(CircuitGenerator.__getActiveCircuitGenerator(), new int[]{8}, 256);
    new_state = (UnsignedInteger[]) UnsignedInteger.createZeroArray(CircuitGenerator.__getActiveCircuitGenerator(), new int[]{8}, 256);
  ...
  public UnsignedInteger[] new_state;
  ...
  public void outsource() {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      branch4[i].add(leafNode[j], new Leaf(new_data[j].copy(256)));
      j = j + 1;
    }
  ...
  public UnsignedInteger[] pointMultiply(UnsignedInteger[] p1, UnsignedInteger n, UnsignedInteger a, UnsignedInteger mult) {
    ...
    for (int i = binMult.length - 1; i >= 0; i--) {
      {
        Bit bit_a0o0xb = bittes[i].copy();
        boolean c_a0o0xb = CircuitGenerator.__getActiveCircuitGenerator().__checkConstantState(bit_a0o0xb);
        if (c_a0o0xb) {
          if (bit_a0o0xb.getConstantValue()) {
            if (flag == false) {
              flag = true;
              binMultLen = i;
            }
          ...
          ConditionalScopeTracker.push(bit_a0o0xb);
          if (flag == false) {
            flag = true;
            binMultLen = i;
          }
    ...
    for (int c = binMultLen; c >= 0; c--) {
      {
        Bit bit_a0q0xb = binMult[c].isEqualTo(num[0]).copy();
        boolean c_a0q0xb = CircuitGenerator.__getActiveCircuitGenerator().__checkConstantState(bit_a0q0xb);
        if (c_a0q0xb) {
          if (bit_a0q0xb.getConstantValue()) {
            if (set) {
          ...
          ConditionalScopeTracker.pushMain();
          ConditionalScopeTracker.push(bit_a0q0xb);
          if (set) {
            val = pointAddition(val, doubledP, n.copy(64));
          } else {
            val = doubledP;
  ...
  public UnsignedInteger modInv(UnsignedInteger b, UnsignedInteger q, UnsignedInteger p) {
    ...
    for (int i = qq.length - 1; i >= 0; i--) {
      {
        Bit bit_a0h0gc = qq[i].copy();
        boolean c_a0h0gc = CircuitGenerator.__getActiveCircuitGenerator().__checkConstantState(bit_a0h0gc);
        if (c_a0h0gc) {
          if (bit_a0h0gc.getConstantValue()) {
            if (flag == false) {
              flag = true;
              fl = i;
            }
          ...
          ConditionalScopeTracker.pushMain();
          ConditionalScopeTracker.push(bit_a0h0gc);
          if (flag == false) {
            flag = true;
            fl = i;
          }
    ...
    for (int i = 0; i <= fl; i++) {
      {
        Bit bit_a0j0gc = qq[i].copy();
        boolean c_a0j0gc = CircuitGenerator.__getActiveCircuitGenerator().__checkConstantState(bit_a0j0gc);
        if (c_a0j0gc) {
          if (bit_a0j0gc.getConstantValue()) {
          ...
          ConditionalScopeTracker.pushMain();
          ConditionalScopeTracker.push(bit_a0j0gc);
          ans.assign(ans.mul(b).mod(p), 64);
