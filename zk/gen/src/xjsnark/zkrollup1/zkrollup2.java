...
public class zkrollup2 extends CircuitGenerator {
  ...
  public zkrollup2() {
    super("zkrollup2");
    __generateCircuit();
    this.__evaluateSampleRun(new SampleRun("Sample_Run2", true) {
      public void pre() {
        ...
        new_data[0].mapValue(new BigInteger("0241270330934425800749994695040591555985320474282562705731155692823696949760"), CircuitGenerator.__getActiveCircuitGenerator().__getCircuitEvaluator());
        new_data[1].mapValue(new BigInteger("79064099132541279051009550594303458602387440456737817675527928952047167196"), CircuitGenerator.__getActiveCircuitGenerator().__getCircuitEvaluator());
  ...
  public void __init() {
    ...
    leaf = (UnsignedInteger[]) UnsignedInteger.createZeroArray(CircuitGenerator.__getActiveCircuitGenerator(), new int[]{2}, 256);
    leafNode = new Leaf[2];
    new_data = (UnsignedInteger[]) UnsignedInteger.createZeroArray(CircuitGenerator.__getActiveCircuitGenerator(), new int[]{2}, 256);
  ...
  public void outsource() {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      branch6[i].add(leafNode[j], new Leaf(new_data[j].copy(256)));
      j = j + 1;
    }
  ...
  public UnsignedInteger[] pointMultiply(UnsignedInteger[] p1, UnsignedInteger n, UnsignedInteger a, UnsignedInteger mult) {
    ...
    for (int i = binMult.length - 1; i >= 0; i--) {
      {
        Bit bit_a0o0wb_2 = bittes[i].copy();
        boolean c_a0o0wb_2 = CircuitGenerator.__getActiveCircuitGenerator().__checkConstantState(bit_a0o0wb_2);
        if (c_a0o0wb_2) {
          if (bit_a0o0wb_2.getConstantValue()) {
            if (flag == false) {
              flag = true;
              binMultLen = i;
            }
          ...
          ConditionalScopeTracker.push(bit_a0o0wb_2);
          if (flag == false) {
            flag = true;
            binMultLen = i;
          }
    ...
    for (int c = binMultLen; c >= 0; c--) {
      {
        Bit bit_a0q0wb_2 = binMult[c].isEqualTo(num[0]).copy();
        boolean c_a0q0wb_2 = CircuitGenerator.__getActiveCircuitGenerator().__checkConstantState(bit_a0q0wb_2);
        if (c_a0q0wb_2) {
          if (bit_a0q0wb_2.getConstantValue()) {
            if (set) {
          ...
          ConditionalScopeTracker.pushMain();
          ConditionalScopeTracker.push(bit_a0q0wb_2);
          if (set) {
            val = pointAddition(val, doubledP, n.copy(64));
          } else {
            val = doubledP;
  ...
  public UnsignedInteger modInv(UnsignedInteger b, UnsignedInteger q, UnsignedInteger p) {
    ...
    for (int i = qq.length - 1; i >= 0; i--) {
      {
        Bit bit_a0h0fc_2 = qq[i].copy();
        boolean c_a0h0fc_2 = CircuitGenerator.__getActiveCircuitGenerator().__checkConstantState(bit_a0h0fc_2);
        if (c_a0h0fc_2) {
          if (bit_a0h0fc_2.getConstantValue()) {
            if (flag == false) {
              flag = true;
              fl = i;
            }
          ...
          ConditionalScopeTracker.pushMain();
          ConditionalScopeTracker.push(bit_a0h0fc_2);
          if (flag == false) {
            flag = true;
            fl = i;
          }
    ...
    for (int i = 0; i <= fl; i++) {
      {
        Bit bit_a0j0fc_2 = qq[i].copy();
        boolean c_a0j0fc_2 = CircuitGenerator.__getActiveCircuitGenerator().__checkConstantState(bit_a0j0fc_2);
        if (c_a0j0fc_2) {
          if (bit_a0j0fc_2.getConstantValue()) {
          ...
          ConditionalScopeTracker.pushMain();
          ConditionalScopeTracker.push(bit_a0j0fc_2);
          ans.assign(ans.mul(b).mod(p), 64);
