...
var AggregateStart = 0
var RequireAggregate = 0
var HandleAggregate = 0
...
func (m *Sealing) handleCommitting(ctx statemachine.Context, sector SectorInfo) error {
	...
	if _, err := os.Stat("paths/aggregate_number"); os.IsNotExist(err) {
		log.Infof("no need to aggregate.")
	} else {
		contentBytes, _ := ioutil.ReadFile("paths/aggregate_number")
		contentStr := strings.TrimSpace(string(contentBytes))
		RequireAggregate, _ = strconv.Atoi(contentStr)
		AggregateStart = int(sector.SectorNumber)
		_ = os.Remove("paths/aggregate_number")
	}

	file, _ := os.Open("paths/filepath" + strconv.Itoa(int(sector.SectorNumber)))
	defer file.Close()
	path, _ := io.ReadAll(file)

	sealed_file, _ := os.OpenFile(string(path), os.O_RDWR|os.O_CREATE, 0644)
	pc1o, _ := io.ReadAll(sealed_file)

	PoS_Generation(pc1o, sector.SeedValue, *sector.CommR, int(sector.SectorNumber))

	if RequireAggregate != 0 {
		if (RequireAggregate + AggregateStart - 1) == int(sector.SectorNumber) {

			log.Info("start to aggregate")
			JavaCmd := "java"
			args1 := []string{
				"-cp",
				"zk/gen/bin:zk/gen/xjsnark_backend.jar",
				"xjsnark.zkrollup1.zkrollup4",
			}
			cmd1 := exec.Command(JavaCmd, args1...)
			_ = cmd1.Run()
			log.Info("finish generate parameters")

			VerifyCmd := "./runtime/run_ppzksnark"
			args2 := []string{
				"zk_output/zkrollup4.arith",
				"zk_output/zkrollup4_Sample_Run4.in",
			}
			cmd2 := exec.Command(VerifyCmd, args2...)
			output, _ := cmd2.CombinedOutput()
			fp, _ := os.Create("zk_output/zk_circuit_output")
			defer fp.Close()
			fp.WriteString(string(output))
			log.Info("finish verify")

			const targetRegexPattern = `\(leave\) Call to r1cs_ppzksnark_verifier_strong_IC\t\[(\d+\.\d+)s x\d+\.\d+\]\t.*`
			re := regexp.MustCompile(targetRegexPattern)
			FindFile, _ := os.Open("zk_output/zk_circuit_output")
			scanner := bufio.NewScanner(FindFile)
			for scanner.Scan() {
				line := scanner.Text()
				matches := re.FindStringSubmatch(line)
				if len(matches) > 1 {
					timeStr := matches[1]
					timeValue, _ := strconv.ParseFloat(timeStr, 64)
					f, _ := os.Create("Verify_Agg_" + strconv.Itoa((HandleAggregate + 1)))
					f.WriteString("elapsed time: " + strconv.FormatFloat(timeValue, 'f', -1, 32) + "s\n")
					defer f.Close()
					break
				}
			}
			_ = os.Remove("zk_output/zk_circuit_output")
			HandleAggregate++
			RequireAggregate = 0
			AggregateStart = 0
		}
...
func PoS_Generation(phase1Out storiface.PreCommit1Out, seed abi.InteractiveSealRandomness, root cid.Cid, ID int) []byte {
	var leafs []*Node2
	proof := []byte{}

	//length := len(phase1Out)

	//f, _ := os.Create("Verify_PoS_" + strconv.Itoa((ID - 1)))
	//defer f.Close()
	//f.WriteString("length: " + strconv.FormatInt(int64(length), 10))
	...
	for i := 1; i <= 10; i++ {
		start := time.Now()
		const NodeSize = 32
		NumNodes := len(proof) / NodeSize
		cur := make([]byte, NodeSize)
		copy(cur, proof[:NodeSize])
		for j := 1; j < NumNodes; j++ {
			sibling := proof[NodeSize*j : NodeSize*(j+1)]
			cur = XOR(cur, sibling)
			h := sha256.Sum256(cur)
			cur = h[:]
		}
	...
	Mean := mean(data)
	f.WriteString("elapsed_time: " + strconv.FormatFloat(Mean, 'f', -1, 32) + "s\n")*/
